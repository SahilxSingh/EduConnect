...
// Since Clerk ID is now the primary key in users table, no conversion needed
...
async function fetchUserSummaries(userIds = []) {
  ...
  // Fetch users directly (no more profiles join needed)
  const users = await selectByIds(
    "users",
    "id",
    unique,
...
export const userAPI = {
  async getUser(clerkId) {
    ...
    const studentRow = Array.isArray(data.students)
      ? data.students[0]
      : data.students;
    const teacherRow = Array.isArray(data.teachers)
      ? data.teachers[0]
      : data.teachers;
  ...
  async updateProfile(clerkId, payload) {
    ...
    // Update users table directly (no more separate profiles table)
  ...
  async getTeachers() {
    ...
    return (data || []).map((user) => ({
      ...
      role: user.role,
      name:
        user.name || user.username || user.email?.split("@")[0] || "Teacher",
      username: user.username,
    }));
  },
};
...
export const assignmentAPI = {
  ...
  async getTeacherAssignments(clerkId) {
    const response = await fetch(
      `/api/assignments?teacherId=${encodeURIComponent(clerkId)}`
    );
    const data = await response.json();
  ...
  async getSubmissions(assignmentId) {
    ...
    const summaries = await fetchUserSummaries(
      (data || []).map((submission) => submission.student_id)
    );
...
export const followAPI = {
  async follow(targetUserId, currentUserId) {
    const { followerId, followingId } = resolveFollowParams(
      targetUserId,
      currentUserId
    );
...
export const queryAPI = {
  ...
  async getTeacherQueries(teacherId) {
    ...
    const { data, error } = await supabase
      .from("queries")
      .select(
        "id, student_id, query_text, answer, answered, created_at, answered_at"
      )
    ...
    const summaries = await fetchUserSummaries(
      (data || []).map((query) => query.student_id)
    );
  ...
  async getStudentQueries(studentId) {
    ...
    const { data, error } = await supabase
      .from("queries")
      .select(
        "id, teacher_id, query_text, answer, answered, created_at, answered_at"
      )
      .eq("student_id", internalStudentId)
      .order("created_at", { ascending: false });
    ...
    const summaries = await fetchUserSummaries(
      (data || []).map((query) => query.teacher_id)
    );
...
export const aiAPI = {
  async askDoubtWithGemini({ question, studentId }) {
    ...
    if (!response.ok) {
      const errorMsg = data.details 
        ...
        : data.error || "Failed to get AI answer";
...
export const chatAPI = {
  async startChat({ participants = [], name = null }) {
    ...
    const internalParticipants = await Promise.all(
      participants.map((id) => getInternalUserId(id))
    );
  ...
  async getUserChats(clerkId) {
    ...
    return (chats || [])
      .map((chat) => {
        const chatParticipants = (participants || []).filter(
          (row) => row.chat_id === chat.id
        );
        const participantSummaries = chatParticipants
          .map((row) => summaries.get(row.user_id))
          .filter(Boolean);
        const otherParticipant =
          participantSummaries.find((summary) => summary.userId !== clerkId) ||
          participantSummaries[0] ||
          null;
